...
public:
	...
	template <typename T>
	__attribute__((malloc, warn_unused_result)) T *try_alloc() noexcept {
		return alloc<T, true>();
	}

	//! Allocates memory for an object of the specified type with an explicit size
	template <typename T, bool may_fail = false>
	__attribute__((malloc, warn_unused_result)) T *alloc_n(size_t n) noexcept {
	...
	template <typename T>
	__attribute__((malloc, warn_unused_result)) T *try_alloc_n(size_t n) noexcept {
	...
	__attribute__((malloc, warn_unused_result)) T *alloc_array(size_t n) noexcept {
	...
	__attribute__((malloc, warn_unused_result)) T *try_alloc_array(size_t n) noexcept {
	...
	template <typename T, size_t sentinel_n = 0, bool may_fail = false>
	__attribute__((malloc, warn_unused_result)) T *alloc_array_range(const T *first, const T *last) noexcept {
		static_assert(sentinel_n <= max_size() / sizeof(T));
	...
	template <typename T, size_t sentinel_n = 0>
	__attribute__((malloc, warn_unused_result)) T *try_alloc_array_range(const T *first, const T *last) noexcept {
		return alloc_array_range<T, sentinel_n, true>(first, last);
	}

	//! Allocates memory for a flexible array of the specified type
	template <typename T, size_t sentinel_n = 0, bool may_fail = false>
	__attribute__((malloc, warn_unused_result)) T *alloc_flexible_array(size_t n) noexcept {
	...
	template <typename T, size_t sentinel_n = 0>
	__attribute__((malloc, warn_unused_result)) T *try_alloc_flexible_array(size_t n) noexcept {
	...
	template <typename T, bool may_fail = false>
	__attribute__((malloc, warn_unused_result)) T *alloc_2d_array(size_t width, size_t height) noexcept {
		size_t n;
	...
	template <typename T>
	__attribute__((malloc, warn_unused_result)) T *try_alloc_2d_array(size_t width, size_t height) noexcept {
		return alloc_2d_array<T, true>(width, height);
	}

	//! Allocates memory for a 3 dimensional array of objects of the specified type
	template <typename T, bool may_fail = false>
	__attribute__((malloc, warn_unused_result)) T *alloc_3d_array(size_t width, size_t height, size_t depth) noexcept {
		size_t m, n;
	...
	template <typename T>
	__attribute__((malloc, warn_unused_result)) T *try_alloc_3d_array(size_t width, size_t height, size_t depth) noexcept {
